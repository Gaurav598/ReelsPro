...
/**
 * Login Page (/login) — 3 tarike se login kar sakte ho:
 *
 * 1. Google — "Sign in with Google" button (OAuth)
 * 2. GitHub — "Sign in with GitHub" button (OAuth)
 * 3. Email + Password — Manual credentials form
 *
 * OAuth Flow:
 * → signIn("google") call karo → Google ki website pe redirect hoga
 * → wahan login karo → wapas aayega /api/auth/callback/google pe
 * → NextAuth session bana dega → home page pe redirect
 *
 * Credentials Flow:
 * → signIn("credentials", { email, password }) call karo
 * → NextAuth backend pe check karega → result milega → handle karo
 */
...
export default function LoginPage() {
  ...
  const handleSocialLogin = (provider: string) => {
    ...
    // signIn with redirect: true automatically handles the OAuth flow
